...
router.patch(
  '/:activityId',
  requireUser,
  requiredNotSent({requiredParams: ['name', 'description'], atLeastOne: true}),
  async (req, res, next) => {
    try {
      const {activityId} = req.params;
...
router.delete('/:activityId', requireUser, async (req, res, next) => {
  try {
    const {activityId} = req.params;
    ...
    if(!activityToUpdate) {
      next({
        name: 'NotFound',
        message: `No routine by ID ${activityId}`
      })
    } else {
      const deletedActivity = await destroyActivity(activityId)
      res.send({success: true, ...deletedActivity});
    }
  } catch (error) {
    next(error);
  }
